...
//  2020.10.18
//
//  argv[0] <fci-ip> fps timeout fileOutName
...
int main(int argc, char** argv){
    if (argc < 5)
    {
        std::cout << "Usage: " << argv[0] << " <fci-ip> fps timeout fileOutName" << std::endl;
        return -1;
    }
    // File out name
    ...
    // fps in [1,1000]
    unsigned int fps = std::floor(getDataFromInput(argv[2],1,1000));
    // timeout
    double timeout = getDataFromInput(argv[3],0.0,300);
    try
    {
